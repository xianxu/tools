...
func TestPracticeOutputClassifiesPhysicalRows(t *testing.T) {
	...
	if o.rows[0].background != languageDark || o.rows[len(lines)-1].background != "" {
...
func TestPracticeDefinitionRegionsOverrideMixedProse(t *testing.T) {
	text := "Oxford\nhola hello\n\nfin end\n"
	p := play.Presentation{Text: text, Regions: []play.PresentationRegion{{Start: 0, End: len(text), Background: languageLight}}}
	o := renderPracticeOutput(p, "en", "es", tintPolicy{"en", languageLight}, nil, surfaceProse, "", 30)
	for i := 0; i < 4; i++ {
...
func TestPracticeUnknownProseAndAnswerExclusions(t *testing.T) {
	...
	o := renderPracticeOutput(p, "es", "es", tintPolicy{"es", languageDark}, nil, surfaceProse, "", 30)
	if o.rows[0].background != "" {
	...
	o = renderPracticeOutput(p, "es", "es", tintPolicy{"es", languageDark}, nil, surfaceProse, "", 30)
	cells, _ := rowTestCells(t, paintLanguageRow(o.text, o.rows[0], 30), 30)
	for i, c := range cells {
		want := 236
		if i >= 5 && i < 9 {
			want = -1
...
func TestPracticeChromeOutput(t *testing.T) {
	...
	b.english("next word")
	pal := newPalette(true)
	o := practiceChromeOutput(b.Presentation, deps{lang: "en"}, options{color: true, tintBackground: languageDark}, pal)
	if !strings.Contains(o.text, pal.dim) || o.rows[0].background != languageDark {
