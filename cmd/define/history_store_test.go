...
func TestEditorPersistsThroughDeps(t *testing.T) {
	...
	runEditor(t.Context(), scriptKeys("sycophantic\r"), nil, first.deps, opt, paintInto(&out), nil, finish, &out, &errb)
	...
	runEditor(t.Context(), scriptKeys("syc"), nil, second.deps, opt2, paintInto(&out2), nil, finish2, &out2, &bytes.Buffer{})
