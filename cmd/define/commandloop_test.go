...
func TestRawEditorDispatchesCommands(t *testing.T) {
	...
	var out, errb bytes.Buffer
	code := runEditor(t.Context(), scriptKeys("/help\r"), nil, rig.deps, opt, recordingConsole(&out, &errb, finish))
...
func TestUnknownCommandSuggestsWithoutDefining(t *testing.T) {
	...
	var out, errb bytes.Buffer
	runEditor(t.Context(), scriptKeys("/histry\r"), nil, rig.deps, opt, recordingConsole(&out, &errb, finish))
...
func TestEditorSuggestsFromCommands(t *testing.T) {
	...
	var out, errb bytes.Buffer
	runEditor(t.Context(), scriptKeys("/hel\x03"), nil, rig.deps, opt, recordingConsole(&out, &errb, finish))
...
func TestTypingNarrowsTheMenuAndAWordHidesIt(t *testing.T) {
	...
	runEditor(t.Context(), scriptKeys("syc\x03"), nil, rig.deps, opt, recordingConsole(&out, &errb, finish))
	if strings.Contains(out.String(), "list the commands") {
	...
	out.Reset()
	runEditor(t.Context(), scriptKeys("/zzz\x03"), nil, rig.deps, opt, recordingConsole(&out, &errb, finish))
...
func TestRawEditorSoundChangesTheSession(t *testing.T) {
	...
	runEditor(t.Context(), keySeq(ks...), nil, rig.deps, opt, recordingConsole(&out, &errb, finish))
...
func TestRawEditorPronReplaysThroughTheLoop(t *testing.T) {
	...
	var out, errb bytes.Buffer
	code := runEditor(t.Context(), scriptKeys("jalapeno\r/pron es\r"), nil, rig.deps, opt, recordingConsole(&out, &errb, func() {}))
	if code != 0 {
