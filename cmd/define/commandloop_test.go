...
func TestRawEditorDispatchesCommands(t *testing.T) {
	...
	var out, errb bytes.Buffer
	code := runEditor(t.Context(), scriptKeys("/help\r"), nil, rig.deps, opt, paintInto(&out), nil, finish, &out, &errb)
...
func TestUnknownCommandSuggestsWithoutDefining(t *testing.T) {
	...
	var out, errb bytes.Buffer
	runEditor(t.Context(), scriptKeys("/histry\r"), nil, rig.deps, opt, paintInto(&out), nil, finish, &out, &errb)
...
func TestEditorSuggestsFromCommands(t *testing.T) {
	...
	var out, errb bytes.Buffer
	runEditor(t.Context(), scriptKeys("/hel\x03"), nil, rig.deps, opt, paintInto(&out), nil, finish, &out, &errb)
...
func TestEditorShowsTheCommandMenu(t *testing.T) {
	...
	var out, errb bytes.Buffer
	runEditor(t.Context(), scriptKeys("/\x03"), nil, rig.deps, opt, paintInto(&out), nil, finish, &out, &errb)
...
func TestTypingNarrowsTheMenuAndAWordHidesIt(t *testing.T) {
	...
	runEditor(t.Context(), scriptKeys("syc\x03"), nil, rig.deps, opt, paintInto(&out), nil, finish, &out, &errb)
	if strings.Contains(out.String(), "list the commands") {
	...
	out.Reset()
	runEditor(t.Context(), scriptKeys("/zzz\x03"), nil, rig.deps, opt, paintInto(&out), nil, finish, &out, &errb)
...
func TestRawEditorSoundChangesTheSession(t *testing.T) {
	...
	runEditor(t.Context(), keySeq(ks...), nil, rig.deps, opt, paintInto(&out), nil, finish, &out, &errb)
...
func TestSubmitLeavesNoMenuInTheFrame(t *testing.T) {
	...
	runEditor(t.Context(), keySeq(ks...), nil, rig.deps, opt, view, nil, finish, &out, &errb)
...
func TestRawEditorPronReplaysThroughTheLoop(t *testing.T) {
	...
	code := runEditor(t.Context(), scriptKeys("jalapeno\r/pron es\r"), nil,
		rig.deps, opt, paintInto(&out), nil, func() {}, &out, &errb)
	if code != 0 {
