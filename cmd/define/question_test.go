...
func TestReadsAsQuestion(t *testing.T) {
	for _, tc := range []struct {
		...
		{"long conversational phrase", "difference between sycophantic and obsequious please", true},
		{"reported statement", "so lickspittle is similar to sycophantic", true},
		{"three words stay a miss", "same meaning then", false},
		{"four words fall back", "same meaning then actually", true},
		{"whitespace separates words", "same\tmeaning  then\nactually", true},
