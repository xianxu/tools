...
func TestEditorLoopHighlightsADeckWordOnScreen(t *testing.T) {
	...
	runEditor(t.Context(), scriptKeys("sycophantic"), nil, rig.deps, opt, editorConsole(&out, &errb, finish))
...
func TestEditorLoopHighlightsAWordLookedUpThisSession(t *testing.T) {
	...
	var out, errb bytes.Buffer
	runEditor(t.Context(), scriptKeys("sycophantic\rsycophantic"), nil, rig.deps, opt, editorConsole(&out, &errb, finish))
