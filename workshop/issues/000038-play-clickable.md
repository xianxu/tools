...
| `cross-cutting-refactor` 0.05/0.24 | T1 lift the click registry | the one task that touches a working loop: `runEditor`'s `clicked` closure becomes a call to `playRegion`, rewiring both loops with three named tests held green. Priced against `#41`'s `todaysQuestions` signature-widening (0.24), which is its shape — not its `menu`→`footer` rename (0.12), which is not |
...
| `greenfield-go-module` 0.06/0.28 | T5 the revealed definition carries its regions | **the row the re-read repriced, twice.** Was 0.02/0.10 for "call `writeRendered` and the regions ride along"; a pinned screen's `WriteRegions` now WRAPS, so a region computed before the wrap underlines one word and answers for another. Then `greenfield` rather than `cross-cutting-refactor`, because that primitive's band tops out at 0.20 and this row was sitting on the ceiling while the prose called it the issue's real risk. When the narrative wants more than the band allows, the honest move is the primitive, not the ceiling — priced at `#41`'s comparable new-behaviour rows (0.28/0.32) |
...
| `milestone-review` 0.00/0.30 | the boundary: run + **manual verification** | the plan's close checklist needs an unsandboxed conformance run AND a real-terminal session — click and HEAR the word, `n`, click the headword and `ORIGIN`, page back, quit, check the transcript. The audio step is verifiable no other way, so the label names it |
| `milestone-review` 0.00/0.32 | the boundary: remediation | |

Derivation notes.

**2.18 → 2.38, and the path there is the point.** The old block was measured by
the plan-quality gate as *"provably stale against the tree it will be measured
on"*, with a warning that its errors offset: *"landing near 2.18 for a different
set of reasons produces a calibration row nobody can read later."* The first
re-derivation did land back on 2.18 exactly — three dead rows out, T5 and the
boundary up, netting to zero. The estimate-quality gate then took two of those
rows apart (below), and the total moved. The mapping table is what makes any of
this readable later; without it the arithmetic looks like nothing happened.

**ONE DELIBERATE DEVIATION FROM v3.1, named rather than buried.** The two
`milestone-review` rows are priced 0.30 and 0.32. v2's table gives milestone
review `0.2–0.5` and v3.1 writes `impl=` at 40% of that, so the scaled ceiling is
**0.20** — these rows are 1.5–1.6× above it and are 26% of the total. That is an
evidence-based OVERRIDE of the primitive table, not the model applied as written:
`#41` ran FIVE close rounds against these same files days ago and measured 4.77
against 3.31, with roughly half the total being boundary work. Pricing this
boundary at the model's ceiling would encode a number the adjacent row already
falsified. Stated here so the close reads a deviation as a deviation.
...
- **The two `milestone-review` rows go 0.16/0.20 → 0.30/0.32**, matching `#41`'s
  — see the deviation note above for why that is outside the model. The old pair
  argued from `#30`'s round counts and then priced at half of what the adjacent
  issue priced off the same evidence. One boundary is still one boundary: removing
  T2/T3/T6 does not shrink these.

- **T1 0.12 → 0.24 and T5 0.20 → 0.28, both from the estimate-quality gate.** T1
  is shaped like `#41`'s `todaysQuestions` widening rather than its rename. T5 was
  pinned at `cross-cutting-refactor`'s ceiling while the prose called it the
  issue's real risk; the primitive was wrong, not the number.
...
sends back. On that reading the actual lands near **2.5–3.5h**, which the
re-derivation has now walked most of the way toward rather than away from. The estimate is
